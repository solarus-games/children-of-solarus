...
local current_map, current_rain_mode, previous_rain_mode
...
function rain_manager:on_map_changed()
  ...
  if current_rain_mode == rain_mode and rain_mode == nil then return end
  -- Clear variables.
  rain_surface, flash_surface, dark_surface = nil, nil, nil
  draw_flash_surface = nil
...
function rain_manager:on_draw(dst_surface)
  ...
  if dark_surface then
    dark_surface:draw(dst_surface) -- Draw dark surface.
  end
...
function rain_manager:stop(timers_list)
...
function rain_manager:start_rain_mode(rain_mode)
  ...
  -- Reset other timers.
  self:stop({"drop_timer", "lightning_timer"})
  self:update_darkness() -- Update darkness (fade-out effects included).
  ...
  if rain_mode == "storm" then
    ...
    -- Darkness.
    dark_surface = sol.surface.create(cw, ch)
  ...
  -- Restart other timers.
  self:stop({"drop_frame_timer", "splash_frame_timer"})
...
function rain_manager:update_darkness()
  ...
  self:stop({"darkness_timer"}) -- Destroy old timer.
  -- Start modifying darkness towards the next value.
  timers["darkness_timer"] = sol.timer.start(current_map, 15, function()
    if dark_surface == nil then return false end
    ...
    if darkness == current_darkness then -- Darkness reached.
      ...
      elseif darkness == 0 then -- No storm: stop darkness.
        dark_surface = nil
      end
      return false
    end
