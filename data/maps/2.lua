...
function game_1_man:on_interaction()
  ...
  if playing_game_1 then
    ...
    map:start_dialog("gem_house.game_1.choose_chest")
  else
    ...
    if unauthorized then
      ...
      map:start_dialog("gem_house.game_1.not_allowed_to_play")
    else
      if not already_played_game_1 then
        -- first time: long dialog with the game rules
        map:start_dialog("gem_house.game_1.intro", game_1_question_dialog_finished)
      else
        -- quick dialog to play again
        map:start_dialog("gem_house.game_1.play_again_question", game_1_question_dialog_finished)
      end
    end
  end
end
...
function game_2_man:on_interaction()
  ...
  if playing_game_2 then
    ...
    map:start_dialog("gem_house.game_2.playing")
  else
    -- dialog with the game rules
    map:start_dialog("gem_house.game_2.intro", game_2_question_dialog_finished)
  end
end
...
function game_3_man:on_interaction()
  ...
  if playing_game_3 then
    ...
    map:start_dialog("gem_house.game_3.restart_question", game_3_question_dialog_finished)
  else
    -- see if the player can still play
    ...
    if unauthorized then
      ...
      map:start_dialog("gem_house.game_3.not_allowed_to_play")
    else
      -- game rules
      map:start_dialog("gem_house.game_3.intro", game_3_question_dialog_finished)
    end
  end
end
...
function map:activate_slot_machine(npc)
  ...
  if playing_game_2 then
    ...
    map:start_dialog("gem_house.pay_first")
  end
end
...
function game_1_question_dialog_finished(answer)
  ...
  if answer == 2 then
    ...
    map:start_dialog("gem_house.game_1.not_playing")
  else
    ...
    if map:get_game():get_money() < 20 then
      -- not enough money
      sol.audio.play_sound("wrong")
      map:start_dialog("gem_house.not_enough_money")
      ...
      map:start_dialog("gem_house.game_1.good_luck")
      playing_game_1 = true
    end
  end
end
...
function game_2_question_dialog_finished(answer)
  ...
  if answer == 2 then
    ...
    map:start_dialog("gem_house.game_2.not_playing")
  else
    -- wants to play game 2
    map:start_dialog("gem_house.game_2.choose_bet", game_2_choose_bet_dialog_finished)
  end
end
...
function game_2_choose_bet_dialog_finished(answer)
  ...
  if map:get_game():get_money() < game_2_bet then
    ...
    map:start_dialog("gem_house.game_2.just_paid")
    playing_game_2 = true
...
function game_3_question_dialog_finished(answer)
  ...
  if answer == 2 then
    -- don't want to play the game
    map:start_dialog("gem_house.game_3.not_playing")
  else
    ...
    if map:get_game():get_money() < 10 then
      ...
      map:start_dialog("gem_house.game_3.go", function()
...
function open_game_1_chest(chest)
  ...
  if not playing_game_1 then
    ...
    map:start_dialog("gem_house.pay_first") -- the game man is angry
...
function game_2_timeout()
  ...
  if symbols[1] == symbols[2] and symbols[2] == symbols[3] then
    ...
    if symbols[1] == 0 then -- 3 green gems
      map:start_dialog("gem_house.game_2.reward.green_gems", game_2_give_reward)
      game_2_reward = 5 * game_2_bet
    elseif symbols[1] == 2 then -- 3 blue gems
      map:start_dialog("gem_house.game_2.reward.blue_gems", game_2_give_reward)
      game_2_reward = 7 * game_2_bet
    elseif symbols[1] == 4 then -- 3 red gems
      map:start_dialog("gem_house.game_2.reward.red_gems", game_2_give_reward)
      game_2_reward = 10 * game_2_bet
    elseif symbols[1] == 5 then -- 3 Yoshi
      map:start_dialog("gem_house.game_2.reward.yoshi", game_2_give_reward)
      game_2_reward = 20 * game_2_bet
    else -- other symbol
      map:start_dialog("gem_house.game_2.reward.same_any", game_2_give_reward)
      game_2_reward = 4 * game_2_bet
    end

  elseif green_found and blue_found and red_found then
    -- three gems with different colors
    map:start_dialog("gem_house.game_2.reward.different_gems", game_2_give_reward)
    game_2_reward = 15 * game_2_bet
  else
    map:start_dialog("gem_house.game_2.reward.none", game_2_question_dialog_finished)
    game_2_reward = 0
  end
